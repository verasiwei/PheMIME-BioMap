...
multipartite_network_Server = function(input,output,session,current_description,current_institution,visualize_network,current_phecode,update_network,update_clicked_id,update_info_all,shared_nodes_id_unique){
  ...
  observeEvent(clicked_nodes(),{
    ## find uniquely shared nodes among the selected nodes and send the shared_nodes_unique_click to d3
    ##prepare the upset plot
    upset_dat = tidy_connect %>%
      filter(institution %in% current_institution()) %>%
      filter(from %in% unique(c(clicked_nodes())) | to %in% unique(c(clicked_nodes()))) %>%
    ...
    show_upset_node = unique(c(clicked_nodes()))
    ...
    # Generate all combinations of clicked nodes
    node_combinations <- lapply(1:length(clicked_nodes()), function(n) {
      combn(clicked_nodes(), n, simplify = FALSE)
    }) %>% unlist(recursive = FALSE)
    
    clicked_patterns <- lapply(node_combinations, function(combo) {
    ...
    for (pattern in clicked_patterns) {
      ...
      shared_nodes_unique <- intersection_nodes_unique(update_click, upset_data)
      # } else {
      # update_click <- str_split(pattern, "/")[[1]]
    ...
    session$sendCustomMessage(type = 'updateD3Selection', message = list(updated_ids=clicked_nodes(), 
                                                                         shared_nodes_unique=shared_nodes_unique_list))
  })
